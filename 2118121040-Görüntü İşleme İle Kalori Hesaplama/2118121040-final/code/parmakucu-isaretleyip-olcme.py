# -*- coding: utf-8 -*-
"""
Created on Sun Apr 28 16:25:24 2024
        ...
        if results_hand.multi_hand_landmarks:
            for (idx, hand_landmarks) in enumerate(results_hand.multi_hand_landmarks):
                ...
                if idx == 0:  # Sağ el
                    ...
                    text_x = x1 + 30
                    text_y = y1
                    cv2.putText(image, str(int(right_distance)), (text_x, text_y), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 128), 3)
                elif idx == 1:  # Sol el
                    x1, y1 = int(hand_landmarks.landmark[8].x * image_width), int(hand_landmarks.landmark[8].y * image_height)
                    x2, y2 = int(hand_landmarks.landmark[7].x * image_width), int(hand_landmarks.landmark[7].y * image_height)
                    cv2.circle(image, (x1, y1), 10, (255, 0, 128), cv2.FILLED) 
                    ...
                    text_x = x1 + 30
                    text_y = y1
                    cv2.putText(image, str(int(left_distance)), (text_x, text_y), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 128), 3)           
